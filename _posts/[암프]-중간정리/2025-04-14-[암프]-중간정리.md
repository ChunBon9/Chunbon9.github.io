...
A forgery is successful, if \\(Vrfy_{pk}(m', s') = 1\\) and \\((m', s')\\) was not made through the signing oracle

A signature scheme provides the strong unforgeability if for any ppt adversary \\(A\\), \\(Pr[Vrfy_{pk}(m', s') = 1 | (m', s') \leftarrow A]\\) is negligible
