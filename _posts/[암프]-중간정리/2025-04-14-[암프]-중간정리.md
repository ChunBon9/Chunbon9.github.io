...
For each \\(i \in \{0, 1, ... , n-1\}\\), let \\(i = (b^i_1, b^i_2, ... ,b^i_{log^i_2})_2\\)  
The \\(i\\)-th sender's string \\(x_i = H(S^1_{b^i_1}, S^2_{b^i_2}, ..., S^{log_n}_{b^i_{log_2}})\\) where \\(S^i_k\\) are the random string
