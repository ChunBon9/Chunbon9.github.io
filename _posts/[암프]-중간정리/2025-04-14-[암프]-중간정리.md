...
For each \\( i \in \{0, 1, \dots, n - 1\} \\), let \\( i = (b^{i}_1, b^{i}_2, \dots, b^{i}_{\log_2 n})_2 \\)  
The \\(i\\)-th sender's string \\( x_i = H(S^1_{b^{i}_1}, S^2_{b^{i}_2}, \dots, S^{\log_2 n}_{b^{i}_{\log_2 n}}) \\)  
where \\( S^i_k \\) are the random strings.
