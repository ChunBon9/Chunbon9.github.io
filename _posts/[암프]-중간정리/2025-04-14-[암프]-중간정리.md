...
#### security properties
- correctness: \\(Pr[Ver(pk, m, c, d) = 1 | pk \leftarrow KGen(1^k), m \in M, (c,d) \leftarrow Com(m, pk)] = 1\\)

알고리즘이 제대로 되어 있다면 Ver결과는 1

- binding: If \\(Ver(pk, m, c, d) = 1\\) and \\(Ver(pk, m', c, d') = 1\\), then \\(m, m'\\) with an overwhelming probability
